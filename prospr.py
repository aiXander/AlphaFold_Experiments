...
train_parser.add_argument('-n','--network', help='Which neural network do you want to use to make the predictions.  ProSPr provides the following:  full, no-potts, and no-msa.  full is default', default='full')
...
def main(args):
    ...
    if args.network == "no-potts":
        network_type = 2
        print("Network type set to 2 since we're not using Potts features!")
    dist_pred, dist_loss = domain(args.domain, network, args.stride, network_type=network_type)
...
def build(args):
    ...
    b.join()
    print('building pkl sequence of domain %s..' %str(args.domain))
    s = Sequence(args.domain)
    print('running s.build...')
    print('network arg: %s' %str(args.network))
    s.build(args)
